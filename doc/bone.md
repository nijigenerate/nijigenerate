...
DepthRigRootのInspectorから、標準骨格追加と同様に標準パラメータを追加できるようにする。
...
存在しない場合はMidori-bone.inxと同じmin/maxとkeypointで作成する。既存パラメータがある場合は破壊的に軸を作り替えず、標準keypointと一致することを前提にする。

標準DepthBone transform binding:

- `Face::Yaw-Pitch`: `Head.transform.r.x`, `Head.transform.r.y`
- `Body::Yaw-Pitch`: `Spine.transform.r.x`, `Spine.transform.r.y`, `Chest.transform.r.x`, `Chest.transform.r.y`, `Clavicle.L.transform.r.x`

`Face::Roll` と `Body::Roll` は標準keypointを持つパラメータとして作成する。Midori-bone.inx上の通常ノードや既存Deformerへのbindingは、DepthRig標準パラメータ追加コマンドの責務には含めない。
